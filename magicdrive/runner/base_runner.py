...
from magicdrive.runner.mylora import save_lora_weight, extract_lora_ups_down
...
class BaseRunner:
    ...
    def run(self):
        ...
        # self.cfg.resume_from_checkpoint = "/home/qid/duzhiying/nuscenes/ckpt/magicdrive-t-log/SDv1.5mv-rawbox-t_2024-08-07_13-11_2.0t_0.4.4/weight-E1-S19348/unet"
        ...
        if self.cfg.runner.validation_before_run or self.cfg.validation_only:
            if self.accelerator.is_main_process:
                ...
                with torch.no_grad():
                    self._validation(global_step, save_path)
        ...
        for epoch in range(first_epoch, self.cfg.runner.num_train_epochs):
            for step, batch in enumerate(self.train_dataloader):
                ...
                if self.accelerator.sync_gradients:
                    ...
                    global_step += 1
                    # print(self.cfg.runner.checkpointing_steps)
                    # save and transfer
                    if global_step % self.cfg.runner.checkpointing_steps == 0 and self.accelerator.is_main_process:
                        # print(True)
                        ...
                        state_dict = self.accelerator.unwrap_model(self.unet).state_dict()
                        param_toq = "up_blocks.3.attentions.2.transformer_blocks.0.attn1.to_q.weight"
                        param_animate = "up_blocks.3.motion_modules.2.temporal_transformer.transformer_blocks.0.attention_blocks.1.to_q.weight"

                        # 检查是否在 state_dict 中存在这个参数
                        # print(state_dict)
                        if param_toq in state_dict:
                            param_value = state_dict[param_toq]
                            print(f"参数 '{param_toq}' 的值：\n{param_value}")
                        else:
                            print(f"参数 '{param_toq}' 不存在于模型中")
                        
                        # 检查是否在 state_dict 中存在这个参数
                        if param_animate in state_dict:
                            param_value = state_dict[param_animate]
                            print(f"参数 '{param_animate}' 的值：\n{param_value}")
                        else:
                            print(f"参数 '{param_animate}' 不存在于模型中")
                            
                        del state_dict

                    # validation
                    # print(self.cfg.runner.validation_steps)
                    if self.accelerator.is_main_process:
                        if global_step % self.cfg.runner.validation_steps == 0:
                            sub_dir_name = f"test-{global_step}"
                            save_path = os.path.join(
                                self.cfg.log_root, sub_dir_name
                            )
                            if not os.path.exists(save_path):
                            ...
                            with torch.no_grad():
                                self._validation(global_step, save_path)
                            logging.info(f"Save test result to: {save_path}")
                            # torch.cuda.empty_cache()
        ...
        self.accelerator.wait_for_everyone()
        if self.accelerator.is_main_process:
            save_path = os.path.join(self.cfg.log_root, "the_lastest")
            if not os.path.exists(save_path):
                os.makedirs(save_path)
            self._save_model(save_path)
        self.accelerator.end_training()
    ...
    def _validation(self, step, save_path):
        with torch.no_grad():
            controlnet = self.accelerator.unwrap_model(self.controlnet)
            unet = self.accelerator.unwrap_model(self.unet)
            image_logs = self.validator.validate(
                controlnet, unet, self.accelerator.trackers, step, save_path,
