...
class BaseValidator:
    ...
    def validate(
        ...
        self._my_save_image(step, image_logs, trackers, save_path)
    ...
    def _my_save_image(self, step, image_logs, trackers, save_path):
        save_dir = save_path # 你可以指定你想要保存图像的目录
        ...
        for i, log in enumerate(image_logs):
            ...
            # 保存原始图像
            ori_img_path = os.path.join(save_dir, f"ori_{i}_{validation_prompt}.png")
            Image.fromarray(log["ori_img"]).save(ori_img_path)

            # 保存生成的图像
            for j, gen_img in enumerate(log["gen_img_list"]):
                gen_img_path = os.path.join(save_dir, f"gen_{i}_{validation_prompt}_{j}.png")
                Image.fromarray(gen_img).save(gen_img_path)

            # 保存带有边框的原始图像
            ori_img_wb_path = os.path.join(save_dir, f"ori_{i}_{validation_prompt}_wb.png")
            Image.fromarray(log["ori_img_wb"]).save(ori_img_wb_path)

            # 保存带有边框的生成图像
            for j, gen_img_wb in enumerate(log["gen_img_wb_list"]):
                gen_img_wb_path = os.path.join(save_dir, f"gen_{i}_{validation_prompt}_wb_{j}.png")
                Image.fromarray(gen_img_wb).save(gen_img_wb_path)

            # 保存地图图像
            map_img_path = os.path.join(save_dir, f"map_{i}_{validation_prompt}.png")
            Image.fromarray(map_img_np).save(map_img_path)
