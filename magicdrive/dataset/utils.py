...
def collate_fn_single(
    ...
    # multi-view images
    # len(examples) equals 16? yes
    # print("single_fn:examples_length:", len(examples))
    pixel_values = torch.stack([example["img"].data for example in examples])
    pixel_values = pixel_values.to(
        memory_format=torch.contiguous_format).float()

    # mask
    # only check examples[0] ????
...
def collate_fn(
    ...
    **kwargs,
    ...
    bbox_maxlen = 0
    input_id_max_len = 0
    # len(examples) equals batch_size? assump
    # print("lenght:", len(examples))
    ...
    if bbox_maxlen != 0:
        for ret_dict in ret_dicts:
            ...
            if bboxes_3d_data is None:
                ...
                uncond_cam = nn.Embedding(1, uncond_cam_in_dim[0] * uncond_cam_in_dim[1])
                # the num of len-3 vectors. We use Fourier emb on len-3 vector.
                def uncond_cam_param(repeat_size: Union[List[int], int] = 1):
                    ...
                    param = uncond_cam(torch.LongTensor(
                        [0] * repeat_size_sum))
                    param = param.clone().detach()
                    param = param.reshape(*repeat_size, -1, uncond_cam_in_dim[1])
                    return param
                
                bboxes_3d_data = {}
                ret_dict['kwargs']['bboxes_3d_data'] = {}
                ret_dict['kwargs']['bboxes_3d_data']['cam_params'] = uncond_cam_param([16, 6])
